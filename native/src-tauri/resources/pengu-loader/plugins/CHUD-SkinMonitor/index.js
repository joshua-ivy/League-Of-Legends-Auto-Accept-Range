...
const STATE_EVENT = "chud-skin-monitor-state";
...
function publishSkinState(payload) {
  ...
  window.__chudSkinState = detail;
  try {
    window.__chudCurrentSkin = detail.name;
...
// window.__chudBridge is exposed in start() after port discovery completes,
// so that consumer plugins' waitForBridge() won't resolve until the port is known.
if (typeof window !== "undefined") {
  window.__chudBridgeEmit = sendBridgePayload; // backward compat (available early)
}
...
function setupBridgeSocket() {
  ...
  bridgeSocket.addEventListener("open", () => {
    ...
    window.__chudBridgeEmit = sendBridgePayload;
    _notifyReady();
  });
  ...
  bridgeSocket.addEventListener("message", (event) => {
    ...
    if (data && data.type === "skin-mods-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-skin-mods", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "maps-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-maps", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "fonts-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-fonts", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "announcers-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-announcers", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "category-mods-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-category-mods", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "others-response") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-others", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "champion-locked") {
      window.dispatchEvent(
        new CustomEvent("chud-custom-wheel-champion-locked", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "phase-change" && data.phase === "Lobby") {
      ...
      window.dispatchEvent(new CustomEvent("chud-custom-wheel-reset"));
      return;
    }
...
async function start() {
  ...
  if (typeof window !== "undefined") {
    window.__chudBridge = Object.freeze({
